...
(async()=>{
  for (const d of ['36178','36664','36656']) {
    console.log('\n=== сделка '+d+': crm.item.delivery.list ===');
    j('deliveries', await tc('crm.item.delivery.list',{entityId:Number(d),entityTypeId:2}));
    console.log('=== сделка '+d+': crm.item.payment.list ===');
    j('payments', await tc('crm.item.payment.list',{entityId:Number(d),entityTypeId:2}));
  }
