...
- For owner-only Bitrix diagnostics, never reuse the initial placement `AUTH_ID`: it can already be stale even after reload or a new placement. Use only auth produced after SDK initialization by `BX24.getAuth()`. If browser isolation hides the SDK global, narrowly observe one normal same-origin backend request that the frontend sends after SDK initialization (for example supply orders or stock form-data), take only its `domain` and `accessToken` in memory, and immediately stop observation. Never print, persist, commit, or put the token in shell history. Clear the browser runtime after the single authorized diagnostic request. If no SDK-authenticated request is available, stop and ask the user; do not cycle through initial tokens, reloads, tabs, or webhooks.
- Keep `B24_APP_OAUTH_VAULT=off` until the user explicitly authorizes its production activation and one-time app reauthorization. The operator bearer is server-only: never expose it to frontend, URLs, output, logs, Git, or shell history. The vault must not replace exact owner verification through `user.current`, and its loss must be handled by reauthorization rather than silently falling back to a webhook.
