...
export function loadBackfillDatabaseConfig(env: NodeJS.ProcessEnv = process.env): Exclude<DatabaseConfig, { mode: 'off' }> {
	...
	if (user === String(env['B24_APP_REPAIR_DB_USER'] ?? '').trim()) throw new Error('Backfill database user must differ from repair writer');
	return { ...runtime, user, password };
}
...
export function loadTildaSyncDatabaseConfig(env: NodeJS.ProcessEnv = process.env): Exclude<DatabaseConfig, { mode: 'off' }> {
	...
	const forbiddenUsers = [runtimeUser, env['B24_APP_MIGRATION_DB_USER'], env['B24_APP_BACKFILL_DB_USER'], env['B24_APP_INVENTORY_DB_USER'], env['B24_APP_REPAIR_DB_USER']]
