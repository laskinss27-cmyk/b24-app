...
import {
	applyInventoryIdentityBackfill,
	buildInventoryIdentityBackfillPlan,
	type InventoryIdentityRow,
} from './identity.js';

test('inventory identity plan preserves every legacy number and is stable after assignment', () => {
	const before: InventoryIdentityRow[] = [
	...
	const first = buildInventoryIdentityBackfillPlan(before, '2026-09-05T08:00:00Z');
	const repeated = buildInventoryIdentityBackfillPlan(after, '2026-09-06T08:00:00Z');
...
test('inventory identity plan fails closed on a conflicting public number', () => {
	const plan = buildInventoryIdentityBackfillPlan([
...
class IdentityConnection implements TransferSqlConnection {
	readonly records: InventoryIdentityRow[];
	...
	constructor(records: InventoryIdentityRow[]) { this.records = records.map((row) => ({ ...row })); }
	...
	async query<T = unknown>(sql: string, values: unknown[] = []): Promise<T> {
		...
		if (sql.includes('FROM inventory_identity_checkpoints')) {
		...
		if (sql.includes('FROM inventory_records')) return this.records.map((row) => ({
			id: row.recordId, bitrix_external_id: row.bitrixExternalId, public_id: row.publicId,
		})) as T;
		if (sql.includes('FROM inventory_public_ids')) return [...this.allocator].map(([publicId, legacyId]) => ({
			public_id: publicId, legacy_bitrix_external_id: legacyId,
		})) as T;
		if (sql.startsWith('INSERT INTO inventory_public_ids')) {
			this.allocator.set(Number(values[0]), Number(values[1]));
			return { affectedRows: 1 } as T;
		}
		if (sql.startsWith('UPDATE inventory_records')) {
		...
		if (sql.includes('INSERT INTO inventory_identity_checkpoints')) {
			this.checkpoint = {
...
function pool(connection: IdentityConnection): TransferSqlPool {
	return { async getConnection() { return connection; }, async query() { throw new Error('pool query is not expected'); } };
}

test('inventory identity apply assigns legacy ids atomically and repeats as a no-op', async () => {
	const connection = new IdentityConnection([
		{ recordId: 1, bitrixExternalId: 311, publicId: null },
		{ recordId: 2, bitrixExternalId: 420, publicId: null },
	]);
	const plan = buildInventoryIdentityBackfillPlan(connection.records, '2026-09-05T08:00:00Z');
	const applied = await applyInventoryIdentityBackfill(pool(connection), plan, plan.planHash);
	assert.deepEqual(applied, {
		planHash: plan.planHash, alreadyApplied: false, sourceRecordCount: 2, assignedRecordCount: 2,
	});
	...
	const repeated = await applyInventoryIdentityBackfill(pool(connection), plan, plan.planHash);
	assert.equal(repeated.alreadyApplied, true);
	assert.equal(connection.rollbacks, 1);
	assert.equal(connection.commits, 1);
});

test('inventory identity apply refuses an unapproved hash before SQL', async () => {
	const plan = buildInventoryIdentityBackfillPlan([], '2026-09-05T08:00:00Z');
	await assert.rejects(() => applyInventoryIdentityBackfill({} as TransferSqlPool, plan, '0'.repeat(64)), /does not match the approved plan/);
});
