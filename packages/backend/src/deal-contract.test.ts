...
test('new Shelly supply contract is generated from the supplied legal template', async () => {
	...
	assert.match(text, /Оставшиеся 30% стоимости оборудования оплачиваются Покупателем непосредственно перед передачей Товара Покупателю/);
	assert.match(text, /Передача Товара допускается только после поступления указанной оплаты на расчетный счет Поставщика/);
	assert.match(text, /5\.3\.4\. Передача Товара Покупателю, в том числе при самовывозе/);
	assert.match(text, /осуществляется только после поступления 100% оплаты стоимости соответствующей партии Товара/);
