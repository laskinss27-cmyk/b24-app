	...
	app.post('/api/quicksale/create', async (req, reply) => {
		...
		try {
			...
			if(withStockSelection){
				...
				await validateFreeStock(app,client,erp,items.filter(i=>!services.has(i.productId)).map(i=>({productId:i.productId,qty:i.quantity,fromStore:i.stockTitle})),app.reservationRuntime,{includeCoreReservations:true});
