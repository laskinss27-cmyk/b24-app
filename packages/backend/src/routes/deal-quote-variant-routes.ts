...
async function hasDealQuoteVariantActivity(app: FastifyInstance, client: B24Client, erp: ErpClient, dealId: number): Promise<boolean> {
	if (app.transferSqlWriter?.mode !== 'primary') await ensureTransfersEntity(client);
	const [stages, realizations, supply, transfers] = await Promise.all([
		listDealStages(erp, dealId),
		listDealRealizations(erp, dealId),
		listSupplyRequestsForDeal(erp, dealId),
		loadTransfers(app, client),
	]);
	return stages.length > 0 || realizations.length > 0 || supply.length > 0 || transfers.some((item) => item.dealId === String(dealId));
}
...
export function registerDealQuoteVariantRoutes(
	...
	app.post('/api/deal/variant-create', async (req, reply) => {
		...
		try {
			...
			const selectCreated = !current.enabled && await hasDealQuoteVariantActivity(app, client, erp, dealId);
	...
	app.post('/api/deal/variant-select', async (req, reply) => {
		...
		try {
			...
			if (current.selectedId && current.selectedId !== variantId) {
				if (await hasDealQuoteVariantActivity(app, client, erp, dealId)) {
