...
import { bindDealTabPlacement, bindInventoryMenuPlacement, bindStockMenuPlacement, bindReportBuilderMenuPlacement, reconcilePlacements, DEAL_TAB_PLACEMENT } from '../b24/placement.js';
	...
	app.post('/install', async (req, reply) => {
		...
		if (auth) {
			try {
				...
				const reportBuilderResult = await bindReportBuilderMenuPlacement({ client, publicBaseUrl: app.config.publicBaseUrl });
				app.log.info({ placement: 'LEFT_MENU', status: reportBuilderResult.status }, '[install] report builder menu bound');
