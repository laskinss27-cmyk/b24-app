...
import { ReservationService, type ReservationActor, type ReservationListItem } from '../reservations/sql-service.js';
...
export function registerApiReservationsRoute(app: FastifyInstance, runtime?: ReservationRuntime): void {
	...
	app.post('/api/reservations/list', async (req, reply) => {
		...
		try {
			...
			let named = requests;
			try {
				if (requests.length) named = await enrichItemNames(requireErp(), requests);
			} catch (error) {
				req.log.warn({ error }, 'Reservation item names unavailable; returning SQL records');
			}
			const enriched = await enrichBitrixContext(client, named).catch((error: unknown) => {
				req.log.warn({ error }, 'Reservation Bitrix context unavailable; returning SQL records');
				return named;
			});
			return { ok: true, enabled: true, canWrite: false, requests: enriched };
		} catch (error) { return errorReply(reply, error); }
	});
	...
	app.post('/api/reservations/supply/list', async (req, reply) => {
		...
		try {
			...
			return { ok: true, enabled: service?.enabled ?? false, canWrite: service?.canWrite ?? false, requests: enriched };
		} catch (error) { return errorReply(reply, error); }
	});
