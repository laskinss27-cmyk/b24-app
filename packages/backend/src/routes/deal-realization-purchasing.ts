...
export async function assertDealRealizationPurchasing(
	...
	const items = await erp.list<Record<string, unknown>>('Item', ['name', 'item_name', 'is_stock_item'], [['name', 'in', ids.map(String)]], 0);
	...
	for (const line of goods) {
		...
		const price = dealLinePurchasingPrice(line.productId, line.rate, prices.get(id));
