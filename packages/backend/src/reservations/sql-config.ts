...
export function loadReservationConfig(env: NodeJS.ProcessEnv = process.env): ReservationConfig {
	...
	if (mode === 'active') {
		const forbidden = [runtimeUser, env['B24_APP_MIGRATION_DB_USER'], env['B24_APP_BACKFILL_DB_USER'], env['B24_APP_TILDA_DB_USER'], env['B24_APP_INVENTORY_DB_USER'], env['B24_APP_REPAIR_DB_USER']]
