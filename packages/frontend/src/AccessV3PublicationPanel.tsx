...
export function AccessV3PublicationPanel({ mock, dirty, savedRevision, directory }: { mock: boolean; dirty: boolean; savedRevision: number; directory?: AccessV3Directory }): JSX.Element {
	...
	return <section className="av3-preview" aria-label="Рабочие права отделов и сотрудников">
		...
		<p>Можно применить пять прав каталога: просмотр закупки, изменение розничной цены, изменение закупочной цены, создание товара и редактирование карточки целиком. Другие разделы и складские допуски остаются черновиком.</p>
		<p>Редактирование карточки включает справочные поля, фото, описание и характеристики. Отдельное право «Редактировать описания и характеристики» пока не применяется. Изменение цен существующего товара разрешается отдельно. Создание включает заполнение начальных цен нового товара и сохраняет проверку доступа Битрикса.</p>
		<p>Личное исключение сильнее отдела. Без явного правила действуют прежние проверки. Состав отделов проверяется на каждом запросе. Скрытую закупку существующего товара редактировать нельзя.</p>
