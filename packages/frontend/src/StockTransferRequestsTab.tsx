	...
	return (
		<section>
			<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
				<div><h2 style={{ fontSize: 16, margin: 0 }}>Заказы на перемещение</h2><p style={{ color: 'var(--app-muted)', fontSize: 13, margin: '3px 0 0' }}>{mode === 'supply' ? 'Просьбы точек, по которым еще не созданы перемещения.' : 'Заказ ничего не резервирует и не меняет остатки.'}</p></div>
			...
			{requests === null ? <p>Загрузка…</p> : shown.length === 0 ? <p className="empty">{status === 'pending' ? 'Необработанных заказов нет.' : 'Заказов нет.'}</p> : (
				<table style={{ width: '100%', borderCollapse: 'collapse' }}>
					...
					<tbody>{shown.map((request) => {
						...
						<td style={TD}>{request.createdAt ? new Date(request.createdAt).toLocaleString('ru-RU') : '—'}<div style={{ color: 'var(--app-muted)', fontSize: 12 }}>{request.createdByName || '—'}</div></td>
						...
						<td style={TD}>{TRANSFER_REQUEST_STATUS[request.status]}{request.transferId ? <div style={{ color: 'var(--app-link)', fontSize: 12 }}>Перемещение #{request.transferId}</div> : null}</td>
						<td style={{ ...TD, textAlign: 'right', color: 'var(--app-link)', whiteSpace: 'nowrap' }}>Открыть ›</td>
					</tr>;
					})}</tbody>
				</table>
			)}
			{openRequest && <div style={overlay} onMouseDown={(event) => { if (event.target === event.currentTarget) setOpenRequest(null); }}>
				<div style={{ ...modalCard, maxWidth: 900, maxHeight: 'calc(100vh - 72px)', overflow: 'auto' }}>
					<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
						<div>
							<div style={{ color: 'var(--app-muted)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase' }}>{openRequest.kind === 'supply' ? 'Заявка снабжению' : 'Заказ на перемещение'}</div>
					...
					<table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 14 }}>
						...
						<tbody>{openRequest.kind === 'supply'
							? (openRequest.supplyLines ?? []).map((line, index) => <tr key={`${line.productId ?? 'manual'}-${index}`}>
								<td style={TD}><b>{line.name || (line.productId ? `Товар #${line.productId}` : 'Позиция без названия')}</b>{line.productId ? <div style={{ color: 'var(--app-muted)', fontSize: 12 }}>#{line.productId}</div> : null}{line.link ? <div><a href={line.link} target="_blank" rel="noreferrer" style={{ color: 'var(--app-link)', fontSize: 12 }}>ссылка</a></div> : null}{line.note ? <div style={{ color: 'var(--app-muted)', fontSize: 12 }}>{line.note}</div> : null}</td>
								<td style={{ ...TD, textAlign: 'right', fontWeight: 600 }}>{line.qty}</td>
							</tr>)
							: openRequest.lines.map((line) => <tr key={line.productId}><td style={TD}><b>{line.name || `Товар #${line.productId}`}</b><div style={{ color: 'var(--app-muted)', fontSize: 12 }}>#{line.productId}</div></td><td style={{ ...TD, textAlign: 'right', fontWeight: 600 }}>{line.qty}</td></tr>)}</tbody>
					</table>
					{openRequest.transferId && <div style={{ marginTop: 12, color: 'var(--app-link)', fontSize: 13 }}>Создано перемещение #{openRequest.transferId}</div>}
