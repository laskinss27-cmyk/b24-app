...
import { ACCESS_V3_LIVE_PERMISSIONS, accessV3Permissions, previewAccessV3, resolveAccessV3, type AccessV3Draft, type AccessV3Preview, type AccessV3Response, type AccessV3Value } from '@b24-app/shared';
	...
	return <section className="access-v3" aria-label="Настройка прав отделов и сотрудников">
		...
		{!loaded || !draft ? <p>{busy ? 'Загружаю полный справочник сотрудников, отделов и складов…' : 'Не удалось загрузить настройки. Закройте и откройте окно повторно.'}</p> : <>
			<div className="av3-layout"><aside className="av3-subjects"><label>Найти отдел или сотрудника<input value={search} onChange={e => setSearch(e.target.value)} placeholder="Имя или отдел" /></label>
				...
				<p className="av3-scope-note">Можно применить: три права цен, создание товара и редактирование карточки целиком. Все другие действия — только черновик. «Как у отдела» / «Текущая база» возвращает прежние живые проверки, включая встроенные личные исключения.</p>
				{permissions.filter(p => p.group === group && p.label.toLowerCase().includes(query.toLowerCase())).map(p => {
					...
					return <div className="av3-permission" key={p.id}><div><b>{p.label}</b><small>{(ACCESS_V3_LIVE_PERMISSIONS as readonly string[]).includes(p.id) ? 'Можно применить после подтверждения владельца' : 'Только черновик — пока не применяется'}</small><span className={`av3-result ${resolved.value}`}>{label(resolved.value)} в проекте правил</span><small>{resolved.source}{resolved.conflict ? ' · Конфликт отделов: действует запрет' : ''}</small></div><div className="av3-switch" role="group" aria-label={p.label}>{(['inherit', 'allow', 'deny'] as const).map(value => <button type="button" key={value} disabled={busy || !title} aria-pressed={(explicit ?? 'inherit') === value} onClick={() => update(p.id, value)}>{value === 'inherit' ? kind === 'departments' ? 'Текущая база' : 'Как у отдела' : value === 'allow' ? 'Разрешить' : 'Запретить'}</button>)}</div></div>;
				})}
			</main></div>
