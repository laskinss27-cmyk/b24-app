...
export function SupplyReservationsView({ readOnly = false }: { readOnly?: boolean } = {}): JSX.Element {
	...
	const visibleRequests = useMemo(() => {
		...
		const group = (request: ReservationRequestView): string => request.status === 'pending'
			? 'pending'
			: request.reservationStatus === 'active' || request.reservationStatus === 'shortfall'
				? request.reservationStatus
				: 'closed';
		const expiresAt = (request: ReservationRequestView): number => Date.parse(request.approvedExpiresAt ?? request.requestedExpiresAt) || Number.MAX_SAFE_INTEGER;
		return requests.filter((request) => (statusFilter === 'all' || group(request) === statusFilter)
			&& (!needle || [request.id, request.reservationId, request.dealId, request.dealTitle, request.dealManagerName,
				request.requestedByName, ...request.lines.flatMap((line) => [line.itemName, line.itemCode, line.erpWarehouseName])]
			...
			.sort((left, right) => sortBy === 'newest'
				...
				: sortBy === 'expires_desc'
					? expiresAt(right) - expiresAt(left)
					: expiresAt(left) - expiresAt(right));
	...
	return <div className="supply-reservations">
		...
		<div className="supply-reservation-sql-filters">
			<input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Товар, сделка, менеджер или склад" aria-label="Поиск резервов" />
			<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} aria-label="Статус резерва"><option value="all">Все статусы</option><option value="pending">На согласовании</option><option value="active">Активен</option><option value="shortfall">Уменьшен по остатку</option><option value="closed">Завершён</option></select>
			<select value={sortBy} onChange={(event) => setSortBy(event.target.value)} aria-label="Сортировка резервов"><option value="urgent">Сначала ближайший срок</option><option value="expires_desc">Сначала поздний срок</option><option value="newest">Сначала новые</option></select>
		</div>
	...
	return <section className="supply-proto-card supply-reservation-create">
		...
		<div className="supply-reservation-create-body">
			...
			<section className="supply-reservation-create-section">
				...
				<div className="supply-reservation-product-picker"><StockProductFilter value={props.picked} onChange={props.onPicked} placeholder="Поиск по названию, артикулу или ID" /></div>
