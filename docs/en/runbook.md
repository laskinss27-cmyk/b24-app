...
As of 2026-08-20, the schema and three least-privilege roles have been provisioned. Verified root-only credentials are stored under `/root/b24-app-secrets`; never print them to shell history, logs, or Git. The production backend runs in `B24_APP_DB_MODE=readiness` with a separate runtime account limited to `USAGE + SELECT`; SQL is used only by `/ready` for `SELECT 1`, not by workflow reads or writes. A one-shot migration runner created only `b24_app_schema_migrations`; it has zero rows and there are no domain tables. A fresh dump with external read-back was restored into `b24_app_restore_20260820_112406`; parity passed and guarded cleanup removed the temporary schema. The previous `MODE=off` container is retained as `b24-backend-prev-before-readiness-20260820-1131`. This closes the gate only for metadata readiness, not for future authoritative domain data.
