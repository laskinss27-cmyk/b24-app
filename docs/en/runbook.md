...
As of 2026-08-20, the schema and three least-privilege roles have been provisioned. Verified root-only credentials are stored under `/root/b24-app-secrets`; never print them to shell history, logs, or Git. The production backend runs in `B24_APP_DB_MODE=readiness` with a separate runtime account limited to `USAGE + SELECT`; SQL is used only by `/ready` for `SELECT 1`, not by workflow reads or writes. A separately built one-shot runner from `5b9a6d8` applied four supply identity/graph migrations: production now has five tables, four migration metadata rows and zero domain rows. The running backend remains `596bddb`; no backfill, workflow SQL read/write, deploy or source switch occurred. Post-DDL dump `20260820_131030-b24_app-database.sql.gz` passed external read-back and an isolated five-table restore parity drill; guarded cleanup removed the temporary schema and runner.
