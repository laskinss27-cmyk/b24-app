...
Baseline перед change: focused `26/26`, общий typecheck успешен. После change focused `29/29`, полный backend `215/215`, общий typecheck и `git diff --check` успешны. Commit `4579048` опубликован и развёрнут как `b24-app:4579048` с сохранением `b24-app:9b6b80c` в `b24-backend-prev-before-4579048`.

Седьмой production dry-run в `2026-08-20T18:38:35.406Z` полностью подтвердил прогноз: ERPNext `392`, `ctv_transfers` `110`, `ctv_tr_requests` `5`; 510 documents / 991 lines / 518 links / 705 allocations, 0 errors и 22 warnings. Две последние ошибки стали `historical_source_line_unavailable` без line allocations; итоговый plan hash — `beb0d8563674cefeff40b78fa7969e37e2572c3ec0bf96cdfefcc250cf9b1881`. `readyToApply=true` означает только структурно объяснённый read-only план; это не разрешение на SQL writer, backfill apply или source switch. Независимый post-check подтвердил 4 migration rows и ноль строк во всех четырёх workflow-таблицах.
