...
Локальная, ещё не развёрнутая tombstone-политика создаёт для отсутствующего transfer только документ `source_missing` с ERP reference evidence и без строк. Все ссылки должны исходить из проведённых Stock Entry старше 24 часов; свежая/непроведённая ссылка остаётся blocker против concurrent-scan race. Политика восстанавливает проверяемую document link от Stock Entry, но не угадывает line allocations: отсутствие исходной строки остаётся явным warning. Наличие повреждённой Bitrix-записи блокирует snapshot и не маскируется tombstone.
