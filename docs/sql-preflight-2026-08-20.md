...
## Follow-up: production supply DDL apply

Commit `5b9a6d845900f284a2da23ffbfa17b0d9ce600cb` опубликован fast-forward в `origin/main`; production build checkout обновлён без изменения двух чужих untracked `scripts/day-x-cleanup.*`. Image `b24-app:5b9a6d8` собран отдельно и не развёрнут как backend. Revision label, четыре migration hashes и isolated `--network none` migration tests 4/4 прошли. Build повторил существующие предупреждения о Node/`undici`, deprecated dependencies, четырёх npm audit findings и размере frontend chunk; они не исправлялись в SQL-этапе.

Непосредственно перед DDL scheduled job `20260820_123002-b24_app-database.sql.gz` прошёл checksum/gzip, содержал одну metadata table и имел root-only `.uploaded` с external read-back marker. Production preflight подтвердил `utf8mb4/utf8mb4_unicode_ci`, 1 table / 0 migration rows / 0 target tables, backend `b24-app:596bddb` в `readiness`, `restart_count=0`, health/readiness и network. Первый диагностический preflight был остановлен до runner из-за незакавыченного `|` в операторском `echo`; исправленный полный preflight прошёл, runtime не затрагивался.
