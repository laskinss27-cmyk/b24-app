...
Для ручных заявок `ctv_tr_requests` развёрнут отдельный [mirror/verified-read foundation](sql-transfer-request-mirror-foundation-2026-09-04.md): migrations `0046`–`0049`, append-only нормализованные revisions/lines, fail-closed checkpointed backfill и полная пагинация Bitrix. Exact-hash apply перенёс `16` заявок / `31` строку с точной parity и идемпотентным повтором. Production работает в `WRITE=shadow`, `READ=verified`; обычный runtime compare дал `16/16`, `0` differences и `responseSource=sql`. Bitrix всё ещё читается для каждой проверки и остаётся fallback; режима `primary` намеренно нет.
...
| ручные заявки на перемещение/снаб | Битрикс24 primary writer/live verifier + `b24_app` verified read | `ctv_tr_requests.DETAIL_TEXT`; migrations `0046`–`0049` хранят identity, append-only revisions/lines и ссылки на transfer/task без JSON; parity `16/16` | SQL возвращается только при полном live-match; mismatch/error автоматически возвращает Bitrix |
...
15. Для ручных заявок перемещения/снаба migrations `0046`–`0049`, fail-closed backfill, `WRITE=shadow` и `READ=verified` развёрнуты: `16` заявок / `31` строка, точная initial/runtime parity и автоматический Bitrix fallback. Следующий шаг — проверить первый реальный update/cancel/convert в shadow; только после этого проектировать idempotency/outbox и SQL-primary writer.
