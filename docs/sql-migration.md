...
Для ручных заявок `ctv_tr_requests` migrations `0046`–`0049`, backfill и shadow/verified runtime уже прошли production gate. Реальные create/update/cancel заявки `21664` дали три append-only revisions, а повторные полные чтения подтвердили точный parity `17/17`, `0` differences и SQL response в verified. Следующий [локальный SQL-first change set](sql-transfer-request-primary-write-foundation-2026-09-04.md) добавляет public-ID allocator, idempotency commands и recoverable Bitrix outbox; migrations `0050`–`0056`, identity backfill и source switch ещё не выполнялись в production.
...
| ручные заявки на перемещение/снаб | Битрикс24 primary + SQL shadow; verified-чтение возвращает SQL только при точном parity | migrations `0046`–`0049`: append-only normalized revisions/lines без JSON; production parity `17/17`, `0` differences | SQL-first migrations `0050`–`0056` подготовлены локально, но ещё не применены и primary flags не включены |
