...
На 2026-08-21 выполнены этап 0, schema/readiness-фундамент этапа 1 и read-only моделирование supply mirror этапа 2 без DML. Объединённый runtime commit `740403a` развёрнут в `B24_APP_DB_MODE=readiness` с отдельным credential и только `USAGE + SELECT`; `/ready` выполняет `SELECT 1`. Ручные one-shot runners применили `0001`-`0005`: production содержит metadata, четыре пустые supply identity/graph tables и пустую checkpoint table, 5 migration rows и 0 workflow/checkpoint rows.
