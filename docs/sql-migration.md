...
Для ручных заявок `ctv_tr_requests` migrations `0046`–`0056`, identity backfill и SQL-first runtime прошли production gate. Реальные create/update/cancel заявки `21664` дали три append-only revisions; полный verified gate подтвердил `17/17`, `0` differences. После отдельной canary production переключён на `B24_APP_TRANSFER_REQUEST_SQL_READ=primary` и `B24_APP_TRANSFER_REQUEST_SQL_WRITE=primary`; Bitrix JSON сохранён как compatibility mirror, commands/outbox перед переключением пусты, rollback-контейнер сохранён. Подробности и проверенные recovery gates — в [журнале SQL-first заявок](sql-transfer-request-primary-write-foundation-2026-09-04.md).
...
| ручные заявки на перемещение/снаб | `b24_app` SQL primary для чтения и записи | migrations `0046`–`0056`: append-only normalized revisions/lines, public-ID allocator, idempotency commands и recoverable outbox без SQL JSON; Bitrix остаётся compatibility mirror | доставка mirror зависит от Bitrix, но ошибка остаётся в recoverable outbox и не отменяет SQL-заявку; первый post-switch create/update/cancel проверяется реальным пользовательским действием |
