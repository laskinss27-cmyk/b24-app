...
На 2026-08-21 выполнены этап 0, schema/readiness-фундамент этапа 1, read-only моделирование и первый неавторитетный supply mirror этапа 2. Объединённый runtime commit `740403a` развёрнут в `B24_APP_DB_MODE=readiness` с отдельным credential и только `USAGE + SELECT`; `/ready` выполняет `SELECT 1`. Ручные one-shot runners применили `0001`-`0005`, а отдельно разрешённый DML-only process записал `516` документов, `1002` строки, `527` связей, `716` allocations и один checkpoint. Runtime workflow эти строки не читает и не пишет.
