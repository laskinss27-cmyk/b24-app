...
| ручные заявки на перемещение/снаб | Битрикс24 primary + `b24_app` SQL shadow | `ctv_tr_requests.DETAIL_TEXT`; migrations `0046`–`0049` хранят identity, append-only revisions/lines и ссылки на transfer/task без JSON; initial parity `16/16` | до реальных shadow revisions и повторной parity пользовательский ответ всегда остаётся Bitrix |
...
15. Для ручных заявок перемещения/снаба migrations `0046`–`0049`, fail-closed backfill и `READ/WRITE=shadow` развёрнуты: `16` заявок / `31` строка, точная initial/runtime parity. Следующий шаг — накопить реальные shadow revisions и повторные matches, затем отдельно разрешить `READ=verified`; только после этого проектировать idempotency/outbox и SQL-primary writer.
