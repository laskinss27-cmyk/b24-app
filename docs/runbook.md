...
Параллельный deploy `0162f23` из ветки, основанной на `aabda51`, вернул права отдела снабжения ID `12`, но временно убрал из runtime 22 commits SQL/dry-run истории: internal/public `/ready` стали 404. MariaDB сохранила `6/5/0`, backup/restore/credentials остались целы; `4579048` был корректно сохранён как rollback. Патч чисто перенесён поверх актуальной `main`, старый fixture отдела исправлен после fail-closed полного теста. Итоговый `740403a` прошёл focused `10/10`, backend `221/221`, frontend `117/117`, typecheck, build и canary и опубликован в `origin/main`.

Production работает на `b24-app:740403a`; `B24_APP_DB_MODE=readiness`, internal/public health и `/ready`, официальный ERP read, `/srv/b24-state`, `127.0.0.1:3000`, `unless-stopped`, restart 0 и `erpnext_frappe_network` подтверждены независимо. Image содержит department ID `12`, writer source и `0001`-`0005`, но writer не вызывается. SQL после deploy `6 tables / 5 migrations / rows 0|0|0|0|0`. Rollbacks: `b24-backend-prev-before-740403a` (`0162f23`) и `b24-backend-prev-before-0162f23` (`4579048`), оба exited 0. Полный журнал и посторонние build-наблюдения — в [`sql-supply-mirror-writer-2026-08-21.md`](sql-supply-mirror-writer-2026-08-21.md).
