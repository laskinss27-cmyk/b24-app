...
- удаление в `primary` намеренно fail-closed и не будет включено до отдельной
  реализации SQL tombstone + Bitrix cleanup outbox.
...
запрет `DELETE`/DDL для DML-only пользователя. Временная schema, user и
контейнер после теста удаляются.
...
5. реализовать и проверить SQL tombstone удаления и перевести оставшиеся
   прямые читатели transfer entity на общий storage adapter;
