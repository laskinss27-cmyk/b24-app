...
- В `b24_app` по-прежнему нет доменных таблиц и workflow-данных.
...
Модель ниже реализована локально как четыре последовательных one-statement migration-файла. Они не применялись ни к production, ни к другой MariaDB; это по-прежнему не разрешение создавать таблицы на сервере.
