...
## Второй production mirror refresh

После первого успешного shadow compare старый checkpoint ожидаемо отстал от live workflow. Перед новым DML локальный и внешний retention были по `11/14`; safety job создал двенадцатый dump `20260821_124850-b24_app-database.sql.gz` (`163253` bytes, 6 definitions), прошёл checksum/gzip и Bitrix Disk read-back, IDs `103948/103946`, ничего не удалив. Два независимых owner read-only прохода повторили точный hash `22ad151b5f2881b525d84c687583bcd23948dbc18f66219734f8091abda0f831`, sources `425/119/5`, rows `552/1064/563/753`, `0` errors, `22` warnings и `readyToApply=true`. Focused baseline до DML прошёл `56/56`.

One-shot runner не сохранял plaintext OAuth или полный plan: он открыл зашифрованный vault только в памяти, подтвердил точного владельца, построил свежий plan и потребовал точного ожидаемого hash до открытия SQL pool. State был read-only; отдельный `b24_app_backfill` независимо подтвердил только `SELECT/INSERT/UPDATE`, глобальный `USAGE`, отсутствие table/column grants, grants вне `b24_app` и свободный named lock. Две первые попытки остановились до writer на чисто диагностических MariaDB-несовместимостях: зарезервированном alias `current_user` и отсутствующей `information_schema.ROUTINE_PRIVILEGES`. После каждой counts/hash были прежними, lock свободен, runner/env отсутствовали. Исправлены только одноразовые preflight queries.

Успешный проход от `12:59:00 UTC` одной транзакцией применил hash `22ad…f831`; немедленный повтор того же in-memory plan вернул `alreadyApplied=true`. SQL стал `552|1064|563|753|2|5`, latest snapshot совпал с checkpoint, orphan counts `0|0|0`, lock освобождён. Внешний shell завершился code `1` уже после успешного JSON, потому что финальная проверка увидела env-файл до собственного `EXIT trap`; trap затем удалил файл, runner был `--rm`, а независимый audit подтвердил DML. Apply не повторялся.

Post-apply job создал тринадцатый dump `20260821_125943-b24_app-database.sql.gz` (`173731` bytes, 6 definitions), Disk IDs `103952/103950`. Restore drill только в `b24_app_restore_20260821_125943` подтвердил совпадающие charset/collation, 6 tables, 66 columns, 37 index rows, 40 constraints, 5 FK, 22 CHECK, counts/latest hash и checksums всех шести tables. Restore schema сохранена до отдельного cleanup. Следующий read-only shadow compare дал полный `match`, `0` differences, `0` plan errors и одинаковые `552/1064/563/753`; audit `/root/b24-app-audits/20260821-130225-supply-shadow-report-match.json`, mode `0600`, SHA-256 `8755697d248189ebb4242307c0dd8fcaa4ee6200a73647faf2411427b2f01381`. После этапа focused suite снова прошёл `56/56`; рабочий backend не менялся, shadow flag остался `off`, source switch не выполнялся.
